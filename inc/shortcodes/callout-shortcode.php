...
/**
 * Class ShortcodeCallout
 */
class ShortcodeCallout {
    /**
     * ShortcodeCallout constructor.
     */
    function __construct() {
        add_action( 'init', array( $this, 'kc_shortcode_callout_mapping' ), 99 );
        add_shortcode( 'kc_shortcode_callout', array( $this, 'kc_shortcode_callout_html' ) );
     ...
     * @since 1.0.0
     */
    function kc_shortcode_callout_mapping() {

        if ( function_exists( 'kc_add_map' ) ) {
            kc_add_map(
                array(

                    'kc_shortcode_callout' => array(
                        'name'        => __( 'Shortcode Call Out', 'msteele' ),
                        'description' => __( 'Display the shortcode', 'msteele' ),
                        'icon'        => '',
                        'category'    => 'Content',
                        'params'      => array(
                            'General'       => array(
                                array(
                                    ...
                                    'label'       => 'Shortcode',
                                    'type'        => 'text',
                                    'description' => 'Paste the shortcode without brackets',
                                )
                            ),
                            'Customization' => array(
                                array(
     ...
     * @since 1.0.0
     */
    function kc_shortcode_callout_html( $atts ) {
        // params extraction
        extract(
            shortcode_atts(
                array(
        ...
        // start the output
        ob_start(); ?>
        <div class="shortcode-callout <?php echo $atts['class']; ?>">
            <div class="shortcode-content" <?php echo $animation; ?>>
